...
public class agencySettingsController {
    ...
    public ResponseEntity<?> updateAgencyProfile(
        ...
        try {
            // 1. JSON String ko Model mein convert karna (Aapka original logic)
            ObjectMapper mapper = new ObjectMapper();
            mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
            
            adminAddAgenciesModel updatedData = mapper.readValue(agencyJson, adminAddAgenciesModel.class);
            Optional<adminAddAgenciesModel> agencyOpt = agencyService.getAgencyById(updatedData.getId());
            
            if (agencyOpt.isPresent()) {
                adminAddAgenciesModel existingAgency = agencyOpt.get();
                
                // Fields Update
                existingAgency.setEmail(updatedData.getEmail());
                ...
                return ResponseEntity.ok(Map.of(
                    ...
                    "newEmail", existingAgency.getEmail(),
            ...
            // 🔴 NAYA BLOCK: Agar Email kisi aur ne le rakha hai, toh 400 Error aayega
    ...
    public ResponseEntity<?> requestOtp(@RequestBody Map<String, String> request) {
        try {
            ...
            if (target == null || target.trim().isEmpty()) {
                return ResponseEntity.status(400).body(Map.of("message", "Email/Phone is required"));
            }

            String otp = String.valueOf((int)((Math.random() * 900000) + 100000));
            otpCache.put(target.toLowerCase().trim(), otp); // 👈 Trim aur Lowercase zaroori hai

            if ("email".equalsIgnoreCase(method)) {
                // Yahan try-catch zaroori hai taaki pata chale mail server down hai ya nahi
                emailService.sendOtpEmail(target, otp);
                return ResponseEntity.ok(Map.of("message", "OTP sent to " + target));
            } else {
                return ResponseEntity.ok(Map.of("message", "SMS OTP (Console): " + otp));
            }
        } catch (Exception e) {
            // Agar SMTP error hai toh yahan se message jayega
            return ResponseEntity.status(500).body(Map.of("message", "Email Service Error: " + e.getMessage()));
        }
    }

    @PostMapping("/security/verify-2fa")
    public ResponseEntity<?> verify2fa(@RequestBody Map<String, String> request) {
        // 🔴 FIX 1: Trim aur Lowercase yahan bhi lagao taaki match pakka ho
        String target = request.get("target") != null ? request.get("target").toLowerCase().trim() : "";
        String userOtp = request.get("otp");
        String loggedInEmail = request.get("email") != null ? request.get("email").toLowerCase().trim() : "";
        ...
        // 🔴 Debugging ke liye log zaroori hai
        System.out.println("❌ Verification Failed. Target: " + target + " OTP provided: " + userOtp);
        return ResponseEntity.status(400).body(Map.of("success", false, "message", "Invalid OTP or Session Expired!"));
    }
