...
public class agencyCampaignsController {
    ...
    public ResponseEntity<?> addCampaign(
            ...
            @RequestHeader("X-Agency-Id") Long agencyId // 🔴 NAYA FIX: Security Header Add Kiya
    ) {
        try {
            ...
            // 🔴 NAYA FIX: Forcefully backend se Agency ID set kar rahe hain (100% Secure)
 ...
 // ==========================================
 // 5. FETCH BY CLIENT (For Client Details Page)
 // ==========================================
 // agencyCampaignsController.java ke andar

    @GetMapping("/client/{clientId}")
    public ResponseEntity<?> getCampaignsByClientId(
            @PathVariable Long clientId, 
            @RequestHeader("X-Agency-Id") Long agencyId) {
        
        // CampaignService ka use karke sirf wahi campaigns layein jo is Client aur Agency ki hain
        List<agencyAddCampaignModel> campaigns = campaignService.getByClientIdAndAgency(clientId, agencyId);
        
        // Agar koi campaign nahi milti toh khali list return karein (404 nahi)
        if (campaigns == null) {
