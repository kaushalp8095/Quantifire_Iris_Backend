...
public class agencyDashboardController {
    ...
    public ResponseEntity<?> getDashboardSummary(@RequestHeader("X-Agency-Id") Long agencyId) { 
        ...
        long totalCampaigns = clientMetrics.stream()
                .mapToLong(m -> m.get("totalCampaigns") != null ? ((Number) m.get("totalCampaigns")).longValue() : 0L).sum();
            response.put("totalCampaigns", totalCampaigns); // Sahi 'totalCampaigns' data map kiya
